	...
	var List = Backbone.Marionette.CompositeView.extend({
			...
			case 'anime':
				if (this.collection.state === 'error') {
					return ErrorView.extend({
						retry: true,
						error: i18n.__('The remote ' + App.currentview + ' API failed to respond, please check %s and try again later', '<a class="links" href="http://www.iwatchonline.to/">http://www.iwatchonline.to</a>')
					});
				} else {
					return ErrorView.extend({
		...
		onLoaded: function () {
			if (!isNaN(startupTime)) {
				win.debug('IWO %s startup time: %sms', Settings.version, (window.performance.now() - startupTime).toFixed(3)); // started in database.js;
				startupTime = 'none';
			}
			this.checkEmpty();
			var self = this;
